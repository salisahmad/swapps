...
export default function Edit({ event, items }: PageProps) {
    const initialAdditionalCosts: AdditionalCost[] = event.additional_costs?.map((cost) => ({
        id: cost.id,
        type: cost.type,
        total: String(cost.total),
        notes: cost.notes || '',
    })) || [];
        ...
        const timeout = window.setTimeout(() => {
            ...
            fetch(`${route('items.search')}?${params.toString()}`, {
                ...
                .then((result: Item[]) => {
                    ...
                    const merged = [
                        ...selectedItems,
                        ...result.filter((item) => !selectedIds.has(item.id)),
                    ];
                            ...
                            {data.date && (
                                <div className="rounded-lg border border-amber-100 bg-amber-50 p-3">
                                    ...
                                    {checkingDate ? (
                                        ...
                                        <p className="mt-1 text-sm text-amber-700">Tidak ada client lain di tanggal ini.</p>
                                    ) : (
