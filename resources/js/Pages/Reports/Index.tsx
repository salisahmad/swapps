                ...
                <div className="mx-auto max-w-7xl sm:px-6 lg:px-8">
                    ...
                    <div className="mb-4 flex flex-wrap gap-2">
                        <form method="get" className="flex flex-wrap gap-2">
                            <select name="year" defaultValue={filters.year} className="rounded border-stone-300 text-sm border-stone-700 bg-stone-900 text-stone-300">
                                {years.map((y) => <option key={y} value={y}>{y}</option>)}
                            </select>
                            <select name="month" defaultValue={filters.month} className="rounded border-stone-300 text-sm border-stone-700 bg-stone-900 text-stone-300">
                    ...
                    <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
                        <div className="rounded-lg bg-green-50 p-4 bg-green-900/20">
                            <p className="text-xs text-stone-500 text-stone-400">Pemasukan</p>
                        ...
                        <div className="rounded-lg bg-red-50 p-4 bg-red-900/20">
                            <p className="text-xs text-stone-500 text-stone-400">Pengeluaran</p>
                        ...
                        <div className="rounded-lg bg-blue-50 p-4 bg-blue-900/20">
                            <p className="text-xs text-stone-500 text-stone-400">Profit</p>
                        ...
                        <div className="rounded-lg bg-yellow-50 p-4 bg-yellow-900/20">
                            <p className="text-xs text-stone-500 text-stone-400">Pending</p>
                            <p className="text-2xl font-bold text-yellow-700 text-yellow-300">{formatRupiah(summary.pending)}</p>
                        </div>
                        <div className="rounded-lg bg-white p-4 shadow-sm bg-stone-800">
                            <p className="text-xs text-stone-500 text-stone-400">Booking Baru</p>
                            <p className="text-2xl font-bold text-stone-900 text-white">{summary.new_events}</p>
                        </div>
                        <div className="rounded-lg bg-white p-4 shadow-sm bg-stone-800">
                            <p className="text-xs text-stone-500 text-stone-400">Event Mendatang</p>
                                ...
                                return (
                                    <div key={i} className="flex flex-col items-center gap-1">
                                        <div className="flex items-end gap-1">
                                            <div style={{ height: Math.max(eHeight, 4) }} className="w-4 rounded bg-green-400 bg-green-500" title={`Pemasukan: ${formatRupiah(d.earnings)}`} />
                                            <div style={{ height: Math.max(xHeight, 4) }} className="w-4 rounded bg-red-400 bg-red-500" title={`Pengeluaran: ${formatRupiah(d.expenses)}`} />
                                        </div>
                                        <p className="text-xs text-stone-500 text-stone-400 rotate-0 whitespace-nowrap">{d.month}</p>
                                    </div>
                                );
                            })}
                        </div>
                    ...
                    <div className="mb-6 rounded-lg bg-white p-6 shadow-sm bg-stone-800">
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-stone-200 divide-stone-700">
                                <thead>
                                    <tr>
                                        <th className="px-3 py-2 text-left text-xs font-medium text-stone-500 text-stone-400">Tanggal</th>
                                ...
                                <tbody className="divide-y divide-stone-200 divide-stone-700">
                                    ...
                                    {daily.length === 0 && (
                                        <tr><td colSpan={4} className="px-3 py-4 text-center text-stone-500 text-stone-400">Tidak ada transaksi bulan ini.</td></tr>
                                    )}
                                </tbody>
                            </table>
                        </div>
                    ...
                    {/* Payment List */}
                    <div className="rounded-lg bg-white p-6 shadow-sm bg-stone-800">
                        <h3 className="mb-4 text-lg font-semibold text-stone-900 text-white">📋 Semua Transaksi</h3>
                        <table className="min-w-full divide-y divide-stone-200 divide-stone-700">
                            <thead>
                                <tr>
                                    <th className="px-3 py-2 text-left text-xs font-medium text-stone-500 text-stone-400">Event</th>
                                    <th className="px-3 py-2 text-left text-xs font-medium text-stone-500 text-stone-400">Tanggal</th>
                                    <th className="px-3 py-2 text-left text-xs font-medium text-stone-500 text-stone-400">Jenis</th>
                                    <th className="px-3 py-2 text-right text-xs font-medium text-stone-500 text-stone-400">Jumlah</th>
                                    <th className="px-3 py-2 text-center text-xs font-medium text-stone-500 text-stone-400">Status</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-stone-200 divide-stone-700">
                                {payments.data.map((p) => (
                                    <tr key={p.id}>
                                        ...
                                        <td className="px-3 py-2 text-center">
                                            <span className={`rounded px-2 py-1 text-xs font-semibold ${p.status === 1 ? 'bg-blue-100 text-blue-800 bg-blue-900 text-blue-300' : 'bg-yellow-100 text-yellow-800 bg-yellow-900 text-yellow-300'}`}>
                                                {p.status_name}
                                            </span>
                                        </td>
                                    </tr>
                                ...
                                {payments.data.length === 0 && (
                                    <tr><td colSpan={5} className="px-3 py-4 text-center text-stone-500 text-stone-400">Tidak ada transaksi.</td></tr>
                                )}
                            </tbody>
                        </table>
